...
{ // This function writes a short report about the words
  ...
  // output stream passed to this function.
	...
	for (int i = 0; i < count; i++)
	{
		...
		output << endl;
		totalVows += Words[i].GetNumVowels();
		totalCons += Words[i].GetNumConsonants();
		totalDigs += Words[i].GetNumDigits();
		totalSpecs += Words[i].GetNumSpecialChars();
	}
	// Prints blank dashes
	output << setw (15) << left << " ";
	output << setw (8) << right << "---";
	...
	output << setw (8) << right << totalVows;
	output << setw (8) << right << totalCons;
	output << setw (8) << right << totalDigs;
	output << setw (8) << right << totalSpecs << endl;
