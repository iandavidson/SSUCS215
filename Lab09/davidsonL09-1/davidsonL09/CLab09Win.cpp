...
afx_msg void CLab09Win::OnPaint()
{
	...
	dc.DrawText(Message, CRect(MessageX, MessageY,
		MessageX + 80, MessageY + 16), DT_LEFT);
}
...
afx_msg void CLab09Win::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	{
		CRect oldRect = CRect(MessageX, MessageY,
			MessageX + 80, MessageY + 16);
		MessageX = point.x;
		MessageY = point.y;
		CRect newRect = CRect(MessageX, MessageY,
			MessageX + 80, MessageY + 16);
